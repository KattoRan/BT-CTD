...
Type* compileLValue(void) {
  ...
  eat(TK_IDENT);
  // Kiểm tra định danh đã khai báo và có thể làm LValue (biến, tham số, hoặc tên hàm hiện tại)
  ...
  switch (var->kind) {
    ...
    case OBJ_PARAMETER:
      ...
      break;
    case OBJ_FUNCTION:
      // Gán giá trị cho tên hàm (để trả về kết quả)
...
void compileAssignSt(void) {
  ...
  // Kiểm tra tính nhất quán về kiểu cơ bản (Slide 5)
...
void compileForSt(void) {
  ...
  // Kiểm tra biến điều khiển phải là biến kiểu số nguyên (Slide 6)
  ...
  checkTypeEquality(var->varAttrs->type, exp2Type);
...
void compileArgument(Object* param) {
  // Slide 7: Nếu tham số hình thức là tham biến (VAR) thì đối số phải là một LValue
...
Type* compileFactor(void) {
  ...
  case TK_IDENT:
    ...
    case OBJ_CONSTANT:
      ...
      else if (obj->constAttrs->value->type == TP_FLOAT) type = floatType;
      else if (obj->constAttrs->value->type == TP_STRING) type = NULL; // String constants not fully supported
      break;
    case OBJ_VARIABLE:
...
Type* compileIndexes(Type* arrayType) {
  ...
  while (lookAhead->tokenType == SB_LSEL) {
    eat(SB_LSEL);
    checkArrayType(type); // Kiểm tra đối tượng có phải kiểu mảng không (Slide 10)
    Type* idxType = compileExpression();
    checkIntType(idxType); // Chỉ số mảng phải là kiểu nguyên
    eat(SB_RSEL);
