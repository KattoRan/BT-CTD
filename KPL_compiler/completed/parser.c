...
void genLValueAddress(Object* obj) {
  switch (obj->kind) {
    case OBJ_VARIABLE:
...
void compileAssignSt(void) {
  ...
  /* -------- Parse LHS (KHÔNG gen code) -------- */
  {
    char name[MAX_IDENT_LEN + 1];
    strcpy(name, currentToken->string);
    eat(TK_IDENT);

    lhsObjs[n]  = checkDeclaredLValueIdent(name);
    lhsTypes[n] = getObjectType(lhsObjs[n]);
    n++;
  }
  ...
  /* -------- Parse + gen RHS -------- */
  for (int i = 0; i < n; i++) {
    rhsTypes[i] = compileExpression();            // push value_i
    checkTypeEquality(lhsTypes[i], rhsTypes[i]);
    if (i < n - 1) eat(SB_COMMA);
  }

  /* -------- Gen gán: PHẢI → TRÁI -------- */
  for (int i = n - 1; i >= 0; i--) {
    genLValueAddress(lhsObjs[i]);  // push addr_i
    genSWAP();                     // stack: ... addr_i value_i
    genST();                       // pop addr+value, store
  }
}
