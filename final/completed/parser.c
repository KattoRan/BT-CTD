...
void compileStatement(void) {
  ...
  case KW_REPEAT: compileRepeatSt(); break; // repeat ... until ...
  case SB_SEMICOLON:
  case KW_END:
  case KW_ELSE:
  case KW_UNTIL:   // <-- THÊM DÒNG NÀY
...
Type* compilePower(void) {
  Type* type1 = compileFactor();

  if (lookAhead->tokenType == SB_POWER) {
    eat(SB_POWER);
    Type* type2 = compilePower();   // right associative
    checkIntType(type1);
    checkIntType(type2);
    // CHƯA sinh mã -> chỉ kiểm tra cú pháp + kiểu
    return intType;
  }
...
Type* compileTerm2(Type* argType1) {
  ...
  case SB_MOD:
    eat(SB_MOD);
    checkIntType(argType1);
    argType2 = compilePower();  // dùng power để ưu tiên **
    checkIntType(argType2);
    // genMOD();  // có thể để trống
    resultType = compileTerm2(argType1);
    break;
